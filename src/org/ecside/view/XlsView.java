...
import org.ecside.core.bean.Td;
...
//        encoding = model.getExportHandler().getCurrentExport().getEncoding();
        ...
        try {
	        ...
	        rownum++;
	        rownum+=createTr(sheet, null, trListBefore, totalCol, 0, rownum, model)-1;
...
//        ps.setFitWidth((short) 1);
    ...
    private static int createTr(WritableSheet sheet,CellFormat cellFormat,
    		List trList,int totalCol,int startCol,int startRow,TableModel model) throws RowsExceededException, WriteException{
    	
    	if (trList!=null && trList.size()>0){
    		
    		if (cellFormat==null){
    			cellFormat=(CellFormat)WritableWorkbook.NORMAL_STYLE;
    		}
    				
    		List mergeCells=new ArrayList();
        		for (int rowNo=startRow;rowNo<startRow+trList.size();rowNo++ ){
        			int idx=0;
        			for (int colNo=startCol;colNo<startCol+totalCol;colNo++){
        			Cell cell=sheet.getCell(colNo,rowNo);
        			if (cell!=null && "_blank_".equals(cell.getContents())){
        				continue;
        			}
        			Td tdBean=((Tr)trList.get(rowNo-startRow)).getTd(idx);
        			String title=tdBean.getContent();
    	            Label label=new Label(colNo,rowNo,title,cellFormat); 
    	            sheet.addCell(label);
    	            sheet.setColumnView(colNo, colWidth);
    	            idx++; 	            
        				int ce=tdBean.getColspan()-1;
        				int re=tdBean.getRowspan()-1;
        				if (ce>=1 || re>=1){
        					ce=ce<0?0:ce;
        					re=re<0?0:re;
        					...
        					if (ce<1 && re>=1){
        		        		for (int srowNo=1;srowNo<=re; srowNo++){
        		    	            Label slabel=new Label(colNo,rowNo+srowNo,"_blank_",cellFormat); 
        						...
        						for (int scolNo=1;scolNo<=ce;scolNo++){
        		    	            Label slabel=new Label(colNo+scolNo,rowNo,"_blank_",cellFormat); 
        		    	            sheet.addCell(slabel);
	        		    		...
	        		    		for (int scolNo=1;scolNo<=ce;scolNo++){
	        		        		for (int srowNo=1;srowNo<=re; srowNo++){
	        		    	            Label slabel=new Label(colNo+scolNo,rowNo+srowNo,"_blank_",cellFormat); 
	        		    	            sheet.addCell(slabel);
	        		    	            sheet.setColumnView(colNo+scolNo, colWidth);
	        		        		}
	        		    		}
    ...
    private void createHeader(TableModel model) throws RowsExceededException, WriteException {
	    ...
	    List trList=(List)model.getTable().getAttribute("ExtendTableTrListTop");
        etr=createTr(sheet, cellFormat, trList, columns.size(), cellnum, rownum, model);
        rownum+=etr;
    ...
    public Object afterBody(TableModel model) {
        ...
        try {
            ...
            rownum++;
            rownum+=createTr(sheet, null, trListAfter, totalCol, 0, rownum, model)-1;
